...
#wires = ["R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"] # = distance 159
#wires = ["R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"] #= distance 135
#wires = ["R8,U5,L5,D3", "U7,R6,D4,L4"] # distance = 6
#wires = ["D3,R3", "U4,R2"]
    ...
    def layout(self):
        ...
        for m in self.moves:
            ...
            for s in range(distance):
                total_distance += 1
                self.current_cell = self.current_cell.nextmove(direction)
                if not self.current_cell in self.path:
                    self.path[self.current_cell] = total_distance


wire1 = Wire(wires[0].split(","))
wire1.layout()
wire2 = Wire(wires[1].split(","))
wire2.layout()

cset = set(wire1.path).intersection(wire2.path)
mind = min(wire1.path[c] + wire2.path[c] for c in cset)
print(mind)

for c in cset:
    d = wire1.path[c] + wire2.path[c]
    print(f"{c} - {d}")
...
print(cset)
for c in cset:
    print(">>",grid[1][c] + grid[2][c])


print(len(wire1.path))
exit()

grid = dict()
positions_used = [Cell(0, 0)]
...
for w in wires:
    ...
    wirenum += 1
    wirelength = 1
    grid[wirenum] = defaultdict(int)
    for step in w.split(","):
        direction = step[0]
        stepcount = int(step[1:])
        for s in range(stepcount):
            current_cell = current_cell.nextmove(direction)
            if grid[wirenum][current_cell] != 0 and grid[wirenum][current_cell] < wirelength:
                wirelength = grid[wirenum][current_cell]
            else:
                grid[wirenum][current_cell] = wirelength
            #print(f"{current_cell}/{wirenum}: {grid[wirenum][current_cell]}")
            wirelength += 1
            positions_used.append(current_cell)
    #print([str(_) for _ in grid[wirenum]])
    #for c in grid[wirenum]:
    #    print(grid[wirenum][c])


def printgrid(agrid):
    minx = min(_.x for _ in positions_used)
    maxx = max(_.x for _ in positions_used)
    miny = min(_.y for _ in positions_used)
    maxy = max(_.y for _ in positions_used)

    print(minx, miny, maxx, maxy)

    print("printgrid: ",[str(_) for _ in agrid])
    print(f"2, -3: {agrid[Cell(2, -3)]}")
...
#for c in grid[1]:
#    if grid[2][c] != 0:
#        steps = grid[1][c] + grid[2][c]
#        if not min_steps or steps < min_steps:
#            min_steps = steps
#        print(f"{c} - {steps}")


cset = set([_ for _ in grid[1] if grid[1][_] > 0]).intersection(set([_ for _ in grid[2] if grid[2][_] > 0]))
for c in cset:
    print(">>",grid[1][c] + grid[2][c])
print(len(cset))
min_steps = min([grid[1][c] + grid[2][c] for c in cset])
print(f"Min steps: {min_steps} (must be more than 7468)") 