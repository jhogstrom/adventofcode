...
for w in wires:
    wirelen = 0
    wirenum += 1
    distgrid[wirenum] = defaultdict(int)
    pos = (0, 0)
    for c in w.split(","):
        wiredistance = c[0]
        length = int(c[1:])
        #print(f"{c} => {wiredistance} -- {length}")

        if wiredistance == "U":
            for y in range(pos[1], pos[1] + length):
                cell = (pos[0], y)
                if wirenum in wiregrid[cell]:
                    wirelen = distgrid[wirenum][cell] + 1
                    continue
                distgrid[wirenum][cell] = wirelen
                wirelen += 1
                grid[cell].add("|")
                wiregrid[cell].add(wirenum)
            pos = (pos[0], pos[1] + length)

        if wiredistance == "D":
            for y in range(pos[1], pos[1] - length, -1):
                cell = (pos[0], y)
                if wirenum in wiregrid[cell]:
                    wirelen = distgrid[wirenum][cell] + 1
                    continue
        ...
        if wiredistance == "R":
            for x in range(pos[0], pos[0] + length):
                cell = (x, pos[1])
                if wirenum in wiregrid[cell]:
                    wirelen = distgrid[wirenum][cell] + 1
                    continue
        ...
        if wiredistance == "L":
            for x in range(pos[0], pos[0] - length, -1):
                cell = (x, pos[1])
                if wirenum in wiregrid[cell]:
                    wirelen = distgrid[wirenum][cell] + 1
                    continue
    ...
    for x in range(minx, maxx + 1):
        ...
        if cell in grid:
            if len(grid[cell]) == 2:
                ...
                c = list(grid[cell])[0]
                for w in distgrid:
                    if cell in distgrid[w]:
                        c = str(distgrid[w][cell] % 10)
                        if c == "0": c = "_"
                #s += c
                slist.append(c)
        else:
...
for cell in grid:
    c = grid[cell]
    if len(c) == 2:
        wiredistance = manhattandist((0, 0), cell)

        if (not mind) or wiredistance < mind:
            mind = wiredistance

        steps = distgrid[1][cell] + distgrid[2][cell]
        if not minsteps or steps < minsteps:
            minsteps = steps
        print(f"{cell}, {wiredistance}, {mind}, {distgrid[1][cell]}, {distgrid[2][cell]}, Distance={distgrid[1][cell] + distgrid[2][cell]}")

print(f"min manhattan distance: {mind}")
print(f"min steps: {minsteps} (Note: 7800 is too low)")
