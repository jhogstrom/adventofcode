...
class Creature:
	...
	def floodfill2(self, startcoord):
		mapped = defaultdict()
		def fill(nodes, dist):
			edge = []
			for n in nodes:
				#print("Finding edge for {0}".format(n))
				if (not n in mapped) or (mapped[n] > dist):
					mapped[n] = dist
					edge += [_ for _ in n.adjacentcells() if not self.map.isoccupied(_)]
			return edge

		newnodes = [_ for _ in startcoord.adjacentcells() if not self.map.isoccupied(_)]
		dist = 0
...
#		self.printintermediate(reachabletargets, "@")
		...
		distances = self.floodfill2(self.coord)
		reachabletargets = [_ for _ in targetpositions if _ in distances.keys()]
		...
		distances = self.floodfill2(closesttarget)
...
class Map:
	...
	def isoccupied(self, coord):
		...
		if coord.y >= len(self.grid): return True
		if coord.x >= len(self.grid[coord.y]): return True
		return False
